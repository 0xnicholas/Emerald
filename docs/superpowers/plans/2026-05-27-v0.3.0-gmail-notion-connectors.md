...
async def test_handle_callback_returns_credentials():
    connector = GmailConnector(entity_id="user_123")
    # We mock the httpx call — wrap in a context where we patch _get_api_client
...
class GmailConnector(BaseConnector):
    ...
    async def sync(self, mode: SyncMode = SyncMode.INCREMENTAL) -> SyncResult:
        ...
        try:
            if mode == SyncMode.INCREMENTAL:
                await self._sync_incremental(client, result)
            else:
                await self._sync_full(client, result)
    ...
    async def _sync_full(self, client: httpx.AsyncClient, result: SyncResult) -> None:
        """Full sync — last 30 days of messages."""
    ...
    async def _sync_incremental(self, client: httpx.AsyncClient, result: SyncResult) -> None:
        """Incremental sync — changes since last historyId."""
        # Get last historyId from sync_metadata
        # (In a full implementation, this reads from the DB connector record.
        #  For the connector instance, we read from self._sync_metadata if available.)
        last_history_id = getattr(self, "_last_history_id", None)
        if not last_history_id:
            # No history ID yet — fall back to full sync
...
class NotionConnector(BaseConnector):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                ...
                json={
                    "grant_type": "authorization_code",
                    "code": code,
                    "redirect_uri": "",  # Notion allows empty for token exchange
                },
            )
            response.raise_for_status()
            data = response.json()
    ...
    async def sync(self, mode: SyncMode = SyncMode.INCREMENTAL) -> SyncResult:
        ...
        try:
            ...
            self._last_synced_at = datetime.now(UTC).isoformat()
            logger.info(
    ...
    async def _sync_database(self, client: httpx.AsyncClient, database: dict) -> bool:
        """Extract rows from a Notion database."""
        from emerald.pipeline.orchestrator import PipelineOrchestrator
