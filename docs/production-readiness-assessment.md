...
>
> **总体结论：Emerald 已具备受控环境生产部署的基础条件。** M1（Dockerfile/K8s/OTEL/基准/CI）完成；M2（v0.5.0，按 ADR-0001 重裁剪为「测试卫生 → 独立侧质量套件 → 真实嵌入绝对分报告 → 安全审计」）全部交付；v0.6.0 按 ADR-0004 退役自研连接器（连接能力外包给连接中心 Totem）；**v0.7.0 完成 M3「图谱深化与文档」**（B3 提及层 / B4 多跳推理 / B5 社区遗忘 / B6 重复整合 / A5 文档 overhaul / web 核心循环，发布门 #52 黑盒走查 19/19）。剩余差距：框架集成生态（仅 Pandaria，待用户信号）。性能 SLA/负载压测（D2-D3）**2026-08-29 经用户决议取消**——不做负载测试投入，性能声明自此无压测背书（决策记录见 roadmap 顶部注）。
>
> **可部署性评级：🟢 生产化进行中（推进至 ~85%）。** 建议路径：v0.8.0 Production-Ready Beta（原 M4 已解散：B5/B6 随 v0.7.0 交付、D2/D3 取消；D4 soak / D5 模糊测试 + 部署文档/运维手册直挂 v0.8.0）。
...
| **性能 SLA 验证** | ⚫ | **2026-08-29 决议取消**（用户决议不需要负载测试；画像 <100ms / 搜索 P50/P99 自此无压测背书，性能预期仅存估算值） |
...
| 5 | **性能 SLA 未验证** | ⚫ 已取消（2026-08-29） | 用户决议不做负载测试（D2/D3 取消）。不再是待办 P0；性能主张仅限估算值，对外承诺需另行论证 |
...
| 17 | **keyword search 性能** | ⏳ 未处理 | ~~如 Staging 负载测试显示瓶颈则在 M4 处理~~（压测已取消）；真实部署性能信号触发时处理 |
| 18 | **版本演进策略** | ✅ 已修复 | API 版本统一为 v1 |

**17 项已识别问题中：**
- ✅ 已修复：14 项（82%）
- 🟡 部分修复：2 项（幂等 customId、画像冷缓存）
- ⚫ 已取消：1 项（性能 SLA 验证 — 2026-08-29 用户决议，D2/D3 取消）
- ⏳ 待评估：1 项（keyword search 性能，靠真实部署信号）
...
- 可选启用本地嵌入（fastembed）或 OpenAI/SiliconFlow 网关嵌入（注意：本地嵌入 1536 维锁死为待立项议题）
...
⚠️ **不建议立即部署**，需完成路线图 M5（v0.8.0）：
- v0.8.0（Production-Ready Beta）：72h soak test / 模糊测试 / 部署文档 / 运维手册（原 M4 已解散：B5/B6 随 v0.7.0 交付、D2/D3 负载压测 2026-08-29 取消）
...
| **负载验证** | ❌ 未验证（2026-08-29 决议取消，不再计划） | ✅ 10k 文档/小时 | 不可比：不做压测投入（用户决议） |
...
| **SLA 保障** | ❌ 未定义（负载测试已取消，SLA 承诺需另行论证） | ✅ SaaS SLA | 不可比：无压测背书（用户决议） |
| **版本演进** | 🟢 v1 统一版本 | ✅ v3→v4 演进 | 已完成 |
| **消费者产品** | ❌ 无（Web UI 是产品层 ADR-0003，在范围内但未实现） | ✅ App + 插件 | 定位选择 |
| **连接器/集成** | 🟢 外包给连接中心 Totem（ADR-0004） | ✅ 原生多源 | 架构选择 |

**差距本质**：Emerald 的「骨架」+「核心能力」已对齐 Supermemory，度量体系独立且证据公开（绝对分报告 + 质量套件）。剩余差距集中在「生态成熟度」（真实生产验证——负载压测 2026-08-29 决议取消，靠真实部署信号、框架集成仅 Pandaria）和「产品形态」（无消费者产品，是定位选择）。详见 [`docs/comparison-supermemory.md`](comparison-supermemory.md) v2。
...
| 性能与扩展性 | 🟡 | 能力已 LLM 化 / 三级降级；**性能 SLA 无压测背书（2026-08-29 决议取消负载测试，仅存估算值）** |
...
**综合评级：🟢 生产化进行中（~85%）**——M1 + M2（v0.5.0）+ 连接器退役（v0.6.0）已交付质量套件、绝对分报告、安全审计三项可信度证据；v0.7.0 补齐图谱深度（提及/多跳/社区遗忘/整合）与 web 核心循环（发布门 #52 黑盒走查 19/19）。性能 SLA 验证 2026-08-29 经用户决议取消（无待办 P0 遗留；性能主张仅限估算）。建议路径：**完成 v0.8.0（D4 soak / D5 模糊 / 部署文档 / 运维手册）后达成 Production-Ready Beta**（详见 [`docs/roadmap.md` §10](roadmap.md)）。
