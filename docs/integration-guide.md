...
> 本节记录 v0.3.0 → v0.4.0 期间影响集成路径的重大变更（M1 + M2 全部完成）。详见 [`docs/roadmap.md`](roadmap.md) 与 [`CHANGELOG.md`](../CHANGELOG.md) [0.4.0] 段。
...
| ~~M4 (v0.8.0)~~ | 已解散（2026-08-29）：高级遗忘/记忆整合随 v0.7.0 交付；负载压测经用户决议取消 |
| v0.8.0 | Production-Ready Beta（原 M5 前移）：72h soak / 模糊测试 / 部署文档/运维手册；无 SLA 压测背书（负载测试已取消，性能承诺不发布） |
