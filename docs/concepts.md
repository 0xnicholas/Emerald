...
## 4. 多跳图谱推理（Multi-hop Reasoning，B4）

图谱优势的落地形态：检索不再只靠向量相似度，还可以沿图谱边行走。

### 4.1 三条遍历路径

| 路径 | 边 | 语义 |
|---|---|---|
| **实体中心检索** | `about=` | 按提及（规范形式）返回该实体提及该事物的全部最新记忆，跨所有表层形式——「谷歌」「GOOGLE」与「Google」命中同一组记忆 |
| **共享主体串联** | 记忆→提及→记忆 | 提及同一事物的记忆互为兄弟节点（一跳） |
| **关系链式** | UPDATES / EXTENDS / DERIVES_FROM（双向） | 推导事实在查询其来源时浮现；推导事实再作为来源参与下一跳（D2 推导自 D1 推导自 A → 查 A 时 D2 以 depth 2 浮现） |

### 4.2 关键语义

- **深度**：`search(..., depth=N)`，默认 0（现状不变，显式 opt-in），上限 4；一跳 = 一条边或一次提及桥
- **实体隔离**：每一跳都限定在实体的上下文池内，跨实体永不返回
- **历史节点**：被取代的记忆（`is_latest=False`）仅在沿 UPDATES 被踩到时浮出并标记，从不主动搜历史、从不穿越历史
- **路径透明**：每个多跳结果携带 `depth`（跳数）与 `path`（经过的节点/边）——Agent 可以解释「为什么返回这条」
- **环路安全**：visited 集合——互引记忆不重复、不死循环，每记忆只出现在最浅深度

### 4.3 与 RAG 的区别

多跳推理是**图谱遍历**：沿关系与提及走，带时序与历史语义，结果可解释（路径）。RAG 是**向量相似度**：无状态、无关系。二者共享同一个 `search` 入口，但由完全不同的引擎驱动。
